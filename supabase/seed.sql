...
insert into public.offers(id,offer_identity,variant_id,platform,seller,title,price,rating,sales,shipping,warranty,url,updated_at,match_confidence) select v.id||'-jd','legacy:'||v.id||'-jd',v.id,'京东','品牌旗舰店','官方正品',case when v.product_id='apple-iphone-16-pro' then 7999 else 4999 end,4.8,12000,'免运费','全国联保 1 年','#',now(),.98 from public.product_variants v where v.storage='256GB' on conflict(id) do nothing;
insert into public.offers(id,offer_identity,variant_id,platform,seller,title,price,rating,sales,shipping,warranty,url,updated_at,match_confidence) select v.id||'-tb','legacy:'||v.id||'-tb',v.id,'淘宝','官方旗舰店','官方正品',case when v.product_id='apple-iphone-16-pro' then 7799 else 4799 end,4.8,8700,'免运费','全国联保 1 年','#',now(),.98 from public.product_variants v where v.storage='256GB' on conflict(id) do nothing;
insert into public.offers(id,offer_identity,variant_id,platform,seller,title,price,rating,sales,shipping,warranty,url,updated_at,match_confidence) select v.id||'-pdd','legacy:'||v.id||'-pdd',v.id,'拼多多','百亿补贴','正品补贴',case when v.product_id='apple-iphone-16-pro' then 7599 else 4599 end,4.6,41000,'免运费','全国联保 1 年','#',now(),.91 from public.product_variants v where v.storage='256GB' on conflict(id) do nothing;
